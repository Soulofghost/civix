...
export default function OverviewMap() {
  ...
  // Default coordinates for Kochi, Kerala
  const defaultCoords = [9.9816, 76.2999];

  // Only use complaints that have a valid lat/lng
  ...
  const activeCoords =
    mappableComplaints.length > 0
      ? [mappableComplaints[0].location.lat, mappableComplaints[0].location.lng]
      : defaultCoords;
              ...
              <div className="p-2 min-w-[150px]">
                ...
                <p
                  className={`text-[10px] font-bold uppercase mb-2 ${
                    c.status === 'Resolved' ? 'text-success' : 'text-warning'
                  }`}
                >
                  {c.status}
                </p>
